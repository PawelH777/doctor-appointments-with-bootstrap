...
class Form extends Component {
    ...
    inputChangedHandler = (event, inputName) => {
        const updatedFormElementAttributes = updateObject(this.state.inputs[inputName].attributes, {
            value: event.target.value
        });

        const updatedFormElementValidation = updateObject(this.state.inputs[inputName].validation, {
            ...
            <div className={classes.Form}>
                <form className="m-3">
                    ...
                    <button
                        ...
                        onClick={() => this.props.submitted(this.state.inputs)}>SUBMIT</button>
                </form>
            </div>
        );
    }
