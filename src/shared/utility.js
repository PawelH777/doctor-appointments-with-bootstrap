export const prepareAppointmentsMap = (daysRange) => {
    let appointsmentsMap = {};
    for(let currentDay = 0; currentDay < daysRange; currentDay++){
        const curDate = new Date();
        curDate.setDate(curDate.getDate() + currentDay);
        const dateConvertedToString = curDate.toLocaleDateString();
        const openingHour = 8;
        const closingHour = 20;
        const datesObjects = [];
        for (let currentHour = openingHour; currentHour < closingHour; currentHour++) {
            const date = {
                id: currentHour + dateConvertedToString,
                date: dateConvertedToString,
                hour: currentHour,
                isReserved: false
            }
            datesObjects.push(date);
        }
        appointsmentsMap = {
            ...appointsmentsMap,
            [dateConvertedToString]: datesObjects
        }
    }
...
export const checkValidity = (value, inputs, inputName) => {
    ...
    if(mustMatchRule) {
        ...
        const mustMatchErrorMessageOfToMatchInput = input.validation.rules.mustMatch.errorMessage;
        const valueToMatch = input.attributes.value;
        if(value !== valueToMatch){
            validation.errors.push(mustMatchRule.errorMessage);
        } else if (input.validation.errors.includes(mustMatchErrorMessageOfToMatchInput)) {
...
const filterOutErrorMessage = (input, errorMessage) => {
    ...
    const filteredErrorsFromToMatchInput = errorsFromToMatchInputCopy.filter(err => err !== errorMessage);
